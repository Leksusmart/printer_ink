...
export class CartridgesService {
    ...
    async create(createDto: {
        ...
        const userCheck = await this.databaseService.query(
            `SELECT id FROM public.employers WHERE id = $1 LIMIT 1`,
            [lastchangeby]
        );
        console.log(userCheck.rows[0], userCheck.rows.length)
        if (userCheck.rows.length === 0) {
            throw new BadRequestException(`Пользователь с ID ${lastchangeby} не найден`);
        }
